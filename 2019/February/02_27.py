...
# This looks like a problem that is ripe for solving with recursion. First, let's try to think of a recurrence we can
# use for this problem. We can try some cases:
#
...
# If string starts with zero, then there's no valid encoding. If the string's length is less than or equal to 1,
# there is only 1 encoding. If the first two digits form a number k that is less than or equal to 26,
# we can recursively count the number of encodings assuming we pick k as a letter. We can also pick the first digit
# as a letter and count the number of encodings with this assumption.
