...
def build_word_right_2(matrix, r, c, length):
    row_len = len(matrix[0])
    return ''.join([matrix[r][i] for i in range(c, min(row_len, length))])


def build_word_down_2(matrix, r, c, length):
    col_len = len(matrix)
...
def check_word_down(matrix, r, c, word, length):
    word_len = len(word)
    col_len = len(matrix)
