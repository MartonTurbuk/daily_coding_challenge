...
# table will map keys to nodes in the linked list, and the linked list will be ordered from least recently used to
# most recently used. Then, for set:
#
# First look at our current capacity. If it's < n, then create a node with the val, set it as the head, and add it as
# an entry in the dictionary. If it's equal to n, then add our node as usual, but also evict the least frequently
# used node by deleting the tail of our linked list and also removing the entry from our dictionary. We'll need to
# keep track of the key in each node so that we know which entry to evict. For get:
#
# If the key doesn't exist in our dictionary, then return null. Otherwise, look up the relevant node through the
# dictionary. Before returning it, update the linked list by moving the node to the front of the list. To help us
# out, we can use the following tricks:
#
