...
# Solution. Notice that whenever we add a character, it should ideally match the one on the other side of the string.
...
# speed up using dynamic programming, as usual. We can either memoize our results so that we don't duplicate any
# work, or use a table and do bottom-up programming.
#
# Let's start with memoization. We can keep a cache and store all our results when we compute them in the cache. If
# we come across a string we've seen before, then we just need to look it up in the cache.
...
# However, this is inefficient due to buildup in the call stack. We can build a 2D table instead. We'll store,
# in each index, the shortest palindrome that can be made in the substring defined from i to i + j. Then instead of
# calling ourselves recursively, we'll just look up the values in our table:
