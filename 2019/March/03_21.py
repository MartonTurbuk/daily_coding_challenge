...
# Solution: We can implement encode by iterating over our input string and keeping a current count of whatever the
# current character is, and once we encounter a different one, appending the count (as a string) and the actual
...
# We can implement decode by iterating over the encoded string and checking each character for a digit. If it is,
# then calculate the correct count, and once we find its corresponding character, extend the result with the
