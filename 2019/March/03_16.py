...
# Solution: A relatively easy way to implement this would be to augment each node with an is_locked attribute as well
# as a parent pointer. We can then implement the methods in a straightforward manner:
#
# is_locked simply returns the node's attribute lock searches the node's children and parents for a true is_locked
# attribute. If it is set to true on any of them, then return false. Otherwise, set the current node's is_locked to
# true and return true. unlock simply changes the node's attribute to false. If we want to be safe, then we should
# search the node's children and parents as in lock to make sure we can actually unlock the node, but that shouldn't
# ever happen. While is_locked is O(1) time, lock and unlock will take O(m + h) time where m is the number of nodes
# in the node's subtree (since we have to traverse through all its descendants) and h is the height of the node (
# since we have to traverse through the node's ancestors).
#
# We can improve the performance of lock and unlock by adding another field to the node that keeps tracks of the
# count of locked descendants. That way, we can immediately see whether any of its descendants are locked. This will
# reduce our lock and unlock functions to only O(h). We can maintain this field by doing the following:
#
